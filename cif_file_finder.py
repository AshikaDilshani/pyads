"""
Download CIF files for materials listed in adsorption_data.xlsx, or for one
manually entered material.

Open source used:
- Crystallography Open Database (COD), no API key required.
"""
...
def first_existing_column(columns: Iterable[str], candidates: Iterable[str], label: str) -> str:
...
def cod_search(session: requests.Session, query: str, limit: int, timeout: int) -> list[dict]:
...
def choose_best_cod_result(results: list[dict], material: str, doi: str) -> dict | None:
...
def download_cod_cif(
    ...
    if not looks_like_cif(response.text):
        return None
    if target_path.exists() and looks_like_cif(target_path.read_text(encoding="utf-8", errors="replace")):
        return target_path
...
def make_session(retries: int, backoff: float) -> requests.Session:
    ...
    session.headers.update({"User-Agent": "mistral-pdf-reader-cif-finder/1.0"})
    return session
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", default=str(DEFAULT_INPUT), help="Path to adsorption_data.xlsx or JSON.")
    parser.add_argument("--material", default="", help="Single material name to search without reading Excel.")
    parser.add_argument("--doi", default="", help="Optional DOI for --material searches.")
    parser.add_argument("--title", default="", help="Optional paper title for --material searches.")
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_DIR), help="Folder for downloaded CIF files.")
    parser.add_argument("--report", default=str(DEFAULT_REPORT), help="CSV report path.")
    parser.add_argument("--max-results", type=int, default=5, help="Maximum COD candidates per query.")
    parser.add_argument("--timeout", type=int, default=30, help="HTTP timeout in seconds.")
    parser.add_argument("--delay", type=float, default=1.0, help="Delay between materials in seconds.")
    parser.add_argument("--retries", type=int, default=3, help="HTTP retries for transient COD failures.")
    parser.add_argument("--backoff", type=float, default=1.0, help="Retry backoff factor in seconds.")
    parser.add_argument("--dry-run", action="store_true", help="Print unique material names only.")
