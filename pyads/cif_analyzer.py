...
def dependency_status() -> dict[str, str]:
    """Return installation status of optional scientific libraries."""
...
def require_core_dependencies() -> None:
    """Raise RuntimeError if gemmi or pymatgen are not installed."""
...
def normalize_text(value: str) -> str:
    """Lower-case and strip non-alphanumeric characters for comparison."""
    return re.sub(r"[^a-z0-9]+", " ", value.lower()).strip()


def tokens(value: str) -> set[str]:
    """Return meaningful word tokens from a material name."""
    return {
        token
...
def safe_float(value: Any) -> float | None:
    """Parse a CIF numeric field, returning None for placeholders."""
    text = clean_text(value)
...
def load_download_rows(report_path: Path) -> list[dict[str, str]]:
    """Read all rows from a CIF download report CSV."""
...
def downloaded_cif_rows(cif_dir: Path, report_path: Path) -> list[dict[str, str]]:
    """Return rows for downloaded CIF files, supplemented with local-only files."""
    rows = [
        row
...
def cif_text_metadata(cif_path: Path) -> dict[str, str]:
    """Extract chemical name, formula, and publication title from raw CIF text."""
...
def first_useful_cif_value(values: list[str]) -> str:
    """Return the first non-placeholder CIF value from a list of raw matches."""
    for value in values:
...
def clean_cif_value(value: str) -> str:
    """Strip quotes and reject CIF placeholder values."""
...
def ase_summary(cif_path: Path) -> dict[str, Any]:
    """Read a CIF with ASE and return formula and atom count, if ASE is available."""
...
def material_match_score(
    material: str, metadata: dict[str, str], formula: str
) -> dict[str, Any]:
    """Score how well a downloaded CIF matches the requested material name."""
...
def make_error_row(row: dict[str, str], cif_path: Path, message: str) -> dict[str, Any]:
    """Build a fully-keyed error row for the analysis report."""
    return {
...
def analyze_cif(row: dict[str, str], xrd_dir: Path, wavelength: str) -> dict[str, Any]:
    """Run gemmi, pymatgen, and ASE analysis on one CIF file and score the material match."""
...
def cleanup_old_reports(report_path: Path) -> None:
    """Remove timestamped backup reports from previous runs."""
...
def write_report(rows: list[dict[str, Any]], report_path: Path) -> Path:
    """Write the analysis report to a CSV file and return its path."""
...
def print_dependency_status() -> None:
    """Print the install status of each optional scientific library."""
    for package, status in dependency_status().items():
        print(f"{package}: {status}")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for the CIF analysis tool."""
    parser = argparse.ArgumentParser(description="Analyze downloaded CIF files.")
    parser.add_argument(
        "--cif-dir",
        default=str(DEFAULT_CIF_DIR),
        help="Folder containing downloaded CIF files.",
    )
    parser.add_argument(
        "--download-report",
        default=str(DEFAULT_DOWNLOAD_REPORT),
        help="cif_download_report.csv from cif_finder.",
    )
    parser.add_argument(
        "--report",
        default=str(DEFAULT_ANALYSIS_REPORT),
        help="Output CSV analysis report.",
    )
    parser.add_argument(
        "--xrd-dir",
    ...
    parser.add_argument(
        "--wavelength",
        default="CuKa",
        help="Pymatgen XRD wavelength, e.g. CuKa.",
    )
    parser.add_argument(
        "--check-deps",
        action="store_true",
        help="Only check installed libraries.",
    )
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> None:
    """Entry point for CIF file analysis."""
